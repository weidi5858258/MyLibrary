...
import com.xianglin.station.util.APPManager;
import com.xianglin.station.util.ToastUtils;

/**
 * 未捕获异常处理类
 我的设想：
 应用发生意外crash时，用这个类捕获，并把日志信息保存到一个文件中。
 然后让应用重启，接着把保存的文件信息提取出来上传到服务器，服务器那边
 用邮件或者短信通知相关人员进行修复。
 如何进行修复的想法：
 哪个方法有异常，那么写个类把这个方法给hack掉，打成jar包，在应用启动后，根据
 条件下载这个jar包，然后运行这个jar里的代码。当运行有异常的代码时，就会执行
 这个类里相关方法的代码。
 */
public class CustomCrashHandler implements UncaughtExceptionHandler {
	...
	// CrashHandler实例
	private static final CustomCrashHandler mInstance = new CustomCrashHandler();
	// 程序的Context对象
	...
	private String savaInfoToSD(Context context, Throwable ex) {
		...
		try {
			fileName = XLApplication.APK_LOG_PATH
